...
\subsubsection{Results}
Dopo aver testato differenti tecniche, l'esito migliore è stato raggiunto con la strategia di \textbf{Full Fine-Tuning} che ha ottenuto i seguenti risultati sul test set:
\begin{itemize}
    \item \textbf{Accuracy:} 93.10\%
    \item \textbf{Macro F1-Score:} 89.26\%
...
\subsubsection{Transfer Learning (Frozen)}
...
\subsubsection{In-Context Learning (K-Shot)}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|}
        \hline
        \textbf{Modello}                 & K & \textbf{Accuracy} & \textbf{F1 Macro} \\
        \hline
        \texttt{roberta-base}            & 0 & \textbf{51.15\%}  & \textbf{26.73\%}  \\
        \texttt{bertweet-base}           & 0 & 49.30\%           & 24.63\%           \\
        \texttt{bert-base-uncased}       & 1 & 48.90\%           & 20.35\%           \\
        \texttt{distilbert-base-uncased} & 1 & 48.05\%           & 25.63\%           \\
        \hline
    \end{tabular}
    \caption{Confronto In-Context Learning (K-Shot) tra i modelli}
\end{table}

In tutti i modelli l'ICL produce risultati contenuti, coerentemente con la natura encoder-only delle architetture
analizzate: questi modelli non sono stati progettati per il condizionamento del contesto tipico dell'ICL,
che risulta invece un punto di forza dei modelli decoder generativi.
