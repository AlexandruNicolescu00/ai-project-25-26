...
\begin{itemize}
    ...
    \item \textbf{Approccio Metodologico:} Un confronto sistematico tra diverse metodologie di apprendimento:
          \begin{enumerate}
              \item \textcolor{red}{\textbf{Transfer Learning (Frozen):} Addestramento del solo classificatore mantenendo il corpo del modello congelato.}
              \item \textcolor{red}{\textbf{Complete Fine-Tuning:} Aggiornamento di tutti i pesi del modello per l'adattamento al task. Si specifica inoltre che, su alcuni modelli, si è scelto di utilizzare anche \textbf{LoRA (Low-Rank Adaptation)}, una tecnica PEFT che inietta matrici addestrabili nei layer del modello.}
              \item \textcolor{red}{\textbf{In-Context Learning (Zero/Few-Shot):} Per testare le capacità native senza alcun aggiornamento dei pesi.}
          \end{enumerate}

          \begin{enumerate}
                \item \textcolor{green}{\textbf{In-Context Learning:} In seguito all’addestramento del solo classificatore mantenendo il corpo del modello congelato (Alignment classification head), si è utilizzato il metodo di prompt engineering few-shot con diversi gradi.}
    
                \item \textcolor{green}{\textbf{Complete Fine-Tuning:} Aggiornamento di tutti i pesi del modello per l’adattamento al task.}
        \end{enumerate}
...
\textcolor{green}{Inizialmente è stato eseguito un addestramento del solo classification head che pur essendo computazionalmente leggero e veloce presenta lo svantaggio di congelare il corpo del modello, limitandone drasticamente la capacità di apprendimento su domini complessi al fine di testare la tecnica di In-Context Learning per testare…, scontrandoci tuttavia con i severi limiti imposti dalla context window ristretta tipica di queste architetture.
Successivamente, abbiamo testato il Complete Fine-Tuning, il quale aggiorna la totalità dei pesi garantendo un’ottima accuratezza, ma risultando al contempo lento e molto oneroso in termini di memoria.
}
...
\begin{itemize}
    ...
    \item Numero di betch di training per device
\end{itemize}
...
\textbf{Performance inContext Learning}

In questa sezione si valuta quanto il modello, allineato al task, riesca a performare utilizzando la tecnica di \textit{in-context learning} applicata al test set. 

La valutazione è stata eseguita mediante tre diversi gradi di somministrazione:
...
\subparagraph{One-Shot Performance}
...
\midrule
0 & 0.4575 & 0.5835 & 0.5129 \\
1 & 0.5577 & 0.7367 & 0.6348 \\
2 & 0.0000 & 0.0000 & 0.0000 \\
3 & 0.0000 & 0.0000 & 0.0000 \\
4 & 0.3235 & 0.4911 & 0.3901 \\
...
Class & Precision & Recall & F1 \\
\midrule
0 & 0.5103 & 0.3838 & 0.4381 \\
1 & 0.4350 & 0.9525 & 0.5972 \\
2 & 0.0000 & 0.0000 & 0.0000 \\
3 & 0.0000 & 0.0000 & 0.0000 \\
4 & 0.5610 & 0.1027 & 0.1736 \\
5 & 0.0000 & 0.0000 & 0.0000 \\
\bottomrule
\end{tabular}
\end{table}
...
Class & Precision & Recall & F1 \\
\midrule
0 & 0.4152 & 0.7917 & 0.5447 \\
1 & 0.6433 & 0.5554 & 0.5961 \\
2 & 0.0000 & 0.0000 & 0.0000 \\
3 & 0.0000 & 0.0000 & 0.0000 \\
4 & 0.3425 & 0.4464 & 0.3876 \\
5 & 0.0000 & 0.0000 & 0.0000 \\
\bottomrule
\end{tabular}
\end{table}
...
Il grafico confronta l’F1-score ottenuto nelle diverse configurazioni sperimentali (Five Shot, Three Shot e One Shot) per ciascuna classe. Si osserva che la classe 1 mantiene le prestazioni migliori in tutte le configurazioni, con valori di F1 prossimi a 0.6, confermandosi la categoria più facilmente discriminabile dal modello.

La classe 0 mostra prestazioni intermedie, con un miglioramento nella configurazione Five Shot rispetto alle altre. La classe 4, invece, presenta valori sensibilmente più bassi, evidenziando una maggiore difficoltà del modello nel riconoscere correttamente tale categoria.
...
Class & Precision & Recall & F1 \\
\midrule
0 & 0.9621 & 0.9604 & 0.9612 \\
1 & 0.9406 & 0.9568 & 0.9486 \\
2 & 0.8553 & 0.8176 & 0.8360 \\
3 & 0.9211 & 0.9211 & 0.9211 \\
4 & 0.8692 & 0.9196 & 0.8937 \\
5 & 0.8163 & 0.6061 & 0.6957 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Metriche per classe - Fine-tuning Completo}
\end{figure}
...
\caption{Confusion Matrix - Fine-tuning Completo}
\end{figure}
...
In particolare, la classe 2 mostra la performance migliore con 665 classificazioni corrette, seguita dalla classe 1 con 558 predizioni corrette. Anche le classi 3, 4 e 5 presentano buoni risultati, rispettivamente con 130, 253 e 206 casi corretti, e un livello di errore complessivamente contenuto.

La classe 6 risulta la più critica: pur avendo 40 classificazioni corrette, mostra una confusione significativa soprattutto con la classe 5 (17 casi), oltre a qualche errore verso le classi 1 e 2. Questo suggerisce una possibile somiglianza tra le classi 5 e 6 o una maggiore difficoltà del modello nel distinguere correttamente questa categoria.
...
In questa fase di addestramento siamo stati più cauti, spostandoci di meno e con una minore decadenza dei pesi, il che causa meno rumore. 

Questo riportato di seguito è un addestramento standard, si poteva eseguirne un altro più completo. Una volta scelti i parametri di training, si poteva proseguire utilizzando il devset (training + validation) con una suddivisione casuale tra training set e validation set, ripetere più volte l’addestramento e calcolare media e varianza della training loss nel punto in cui la validation loss raggiungeva il suo minimo. Alla fine si eseguiva un unico training su tutto il devset fermando l’addestramento quando la training loss raggiunge la media delle training loss precedenti. 
...
Class & Precision & Recall & F1 \\
\midrule
0 & 0.4485 & 0.6970 & 0.5458 \\
1 & 0.5234 & 0.8201 & 0.6390 \\
2 & 0.4285 & 0.0189 & 0.0361 \\
3 & 0.0000 & 0.0000 & 0.0000 \\
...
Class & Precision & Recall & F1 \\
\midrule
0 & 0.5140 & 0.1583 & 0.2421 \\
1 & 0.3752 & 0.9784 & 0.5425 \\
2 & 0.2222 & 0.0126 & 0.0238 \\
3 & 0.0000 & 0.0000 & 0.0000 \\
4 & 0.0000 & 0.0000 & 0.0000 \\
5 & 0.0000 & 0.0000 & 0.0000 \\
\bottomrule
...
Class & Precision & Recall & F1 \\
\midrule
0 & 0.4673 & 0.3941 & 0.4276 \\
1 & 0.4647 & 0.8158 & 0.5922 \\
2 & 0.1379 & 0.2516 & 0.1782 \\
3 & 0.0000 & 0.0000 & 0.0000 \\
4 & 0.0000 & 0.0000 & 0.0000 \\
5 & 0.0000 & 0.0000 & 0.0000 \\
\bottomrule
...
Class & Precision & Recall & F1 \\
\midrule
0 & 0.9641 & 0.9707 & 0.9674 \\
1 & 0.9602 & 0.9381 & 0.9490 \\
2 & 0.8095 & 0.8553 & 0.8318 \\
3 & 0.9200 & 0.9200 & 0.9200 \\
4 & 0.8707 & 0.9018 & 0.8860 \\
5 & 0.7377 & 0.6818 & 0.7087 \\
\bottomrule
\end{tabular}
\end{table}
