...
\begin{itemize}
    ...
    \item \textbf{Approccio Metodologico:} Un confronto sistematico tra tre metodologie di apprendimento:
          \begin{enumerate}
              \item \textbf{In-Context Learning (Zero/Few-Shot):} Per testare le capacità native senza aggiornamento dei pesi.
              \item \textbf{Transfer Learning (Frozen):} Addestramento del solo classificatore mantenendo il corpo del modello congelato.
              \item \textbf{LoRA (Low-Rank Adaptation):} Tecnica PEFT che inietta matrici addestrabili nei layer del modello.
          \end{enumerate}
...
Abbiamo scartato l'uso di LLM generativi generalisti (es. GPT-4, Llama-3) per motivi di latenza in inferenza e costi computazionali,
preferendo un'architettura \textbf{Encoder-Only} (\texttt{BERTweet}). I modelli Encoder sono strutturalmente superiori nei task di
classificazione pura grazie all'attenzione bidirezionale che cattura l'intero contesto della frase simultaneamente.

Tra le tecniche di addestramento, la scelta è ricaduta su \textbf{LoRA (Low-Rank Adaptation)} come soluzione primaria, poiché ha
dimostrato di ottenere risultati nettamente migliori rispetto al \textit{Transfer Learning} classico (approccio \textit{Feature Extraction}).
A differenza di quest'ultimo (che congela il corpo del modello limitandone drasticamente la capacità di apprendimento) e del
\textit{Full Fine-Tuning} (che risulta lento e computazionalmente oneroso), LoRA permette di adattare efficacemente le rappresentazioni
interne del modello al task specifico delle emozioni modificando solo poche matrici di peso aggiuntive. Questo garantisce il miglior
compromesso tra modularità, efficienza e prestazioni finali.
...
\subsection{Ablation Study: Analysis of Configurations}
